...
export function ThemeSwitcher({
  ...
  useEffect(() => {
    setMounted(true);
    // Initialize previousOption after mount
    const initialOption = themeOptions.find(
      (opt) => opt.value === (value ?? theme ?? internalValue)
    )?.cOption || null;
    setPreviousOption(initialOption);
  }, []);

  const activeValue = mounted ? ((value ?? theme ?? internalValue) as Theme) : defaultValue;
  ...
  if (!mounted) {
    return (
      <fieldset className="switcher" data-previous="1">
        <legend className="switcher__legend">Choose theme</legend>
        <div className="switcher__wrapper">
          {themeOptions.map((option) => (
            <div key={option.value} className="switcher__option">
              <input
                type="radio"
                id={option.value}
                name="theme"
                disabled
              />
              <label htmlFor={option.value} className="switcher__label">
                {option.icon}
